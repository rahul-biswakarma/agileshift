...
const DisplayFilters = ({type, data, setActiveFilters, activeFilters} : DisplayFiltersProps) => {
    ...
    const [activeFilterData, setActiveFilterData] = useState<{[key:string]:string[]}>({});
    ...
    const modifyFilterActiveState = (index:number) => {
        ...
        if(newArray[index].active === true){
            ...
            let filters:any;
            activeFilterList[type] ? filters = [...activeFilterList[type]] : filters = [];
    ...
    <div>
        <div className="absolute top-[110%] left-0 bg-primary_background_color w-48 rounded-xl p-1 border border-white/20 text-highlight_font_color">
            <div className="flex flex-wrap gap-1 p-2 border-b border-white/10">
                {filterData.filter((column) => column.active === true).map((column, index) => (
                    <button
                        key={index}
            ...
            <div className="flex flex-col gap-1 mt-1 p-1">
                {
                    filterData.map((data, index) => {
                        if(data.active === true){
                            return (
                                <div key={index} onClick={() => modifyFilterActiveState(index)} className="px-2 py-1 text-xs font-semibold bg-Secondary_background_color rounded-lg cursor-pointer border border-white/10">
                                    {data.name}
                                </div>
                            )
                        }else{
                            return (
                                <div key={index} onClick={() => modifyFilterActiveState(index)}  className="px-2 py-1 text-xs font-semibold hover:bg-Secondary_background_color rounded-lg cursor-pointer border border-transparent hover:border-white/10">
                                    {data.name}
                                </div>
                            )
                        }
