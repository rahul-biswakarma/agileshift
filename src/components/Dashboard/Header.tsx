...
const Header = (props: TYPE_HeaderProps) => {
	...
	const unReadNotificationCountFromStore = useAppSelector(
	...
	const [isSettingOptionMenuOpen, setIsSettingOptionMenuOpen] = useState(false);
	...
	const fetchNotificationList = async () => {
		...
		onSnapshot(doc(db, "organizations", organizationId), async (doc) => {
			if (doc.data()) {
				...
				if (
					notificationListFromBackend &&
					notificationListFromBackend.length > 0
				) {
					notificationListFromBackend.forEach(
					...
					const updatedNotificationList = notificationListFromBackend.sort(
						(
							notificationA: TYPE_NOTIFICATION,
							notificationB: TYPE_NOTIFICATION
						) => {
	...
	return (
		<div
			id="header"
			className="p-[0_2rem] h-full min-h-[60px] gap-[2rem] max-h-[61px] flex items-center justify-between border-[2px] border-Secondary_background_color text-white/30 hover:text-purple-500"
		>
			<button
			...
			>
				...
				<p className="uppercase font-fira_code font-[500] whitespace-nowrap">
					{organizationName}
				</p>
			</button>
			<Modal
			...
			<div className="flex gap-[2rem] items-center">
				<div className="relative flex items-center">
					...
					>
						<button
							className={`${
								openOrgList ? "text-white" : "text-white/20"
					...
					>
						<div>
							{openOrgList && (
								<div
									className="absolute min-w-[300px] top-[50px] right-52 rounded-lg font-dm_sans bg-background_color border border-[#444444] 
